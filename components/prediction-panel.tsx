...
export function PredictionPanel({ selectedModel, isModelTrained }: PredictionPanelProps) {
  ...
  const handlePredict = async () => {
    ...
    const probabilities = {
      Dengue: Math.random() * 0.4 + (predictedDisease === "Dengue" ? 0.5 : 0.1),
      Malaria: Math.random() * 0.4 + (predictedDisease === "Malaria" ? 0.5 : 0.1),
      Leptospirosis: Math.random() * 0.4 + (predictedDisease === "Leptospirosis" ? 0.5 : 0.1),
    }
  ...
  return (
    <div className="space-y-6">
      ...
      {prediction && (
        <Card className="p-6 bg-gradient-to-br from-accent/10 to-primary/5 border-accent/30 shadow-lg">
